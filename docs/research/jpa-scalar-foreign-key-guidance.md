...
This guide applies to the issue 3 persistence read path. The project uses Spring Data JPA 4.1.0, Hibernate ORM 7.4.1, and PostgreSQL. Repository-facing service implementations load stored rows and map them to immutable application and calculation values.
...
Use scalar foreign-key fields and explicit bulk queries in repository-facing service implementations. Apply these conditions:
