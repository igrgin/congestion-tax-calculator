...
`CalculationServiceImpl` owns supported-year input failures. It also translates lower lookup, Domain calculation, and stored-content failures into calculation-owned exceptions and preserves their causes. `MissingTaxTimeBandsException`, which means that the stored collection is empty, and `NoMatchingTaxTimeBandException`, which means that no Tax Time Band contains a non-exempt Passage, both become `MissingStoredTaxTimeBandsException`. An invalid City time zone becomes `InvalidStoredCityTimeZoneException`, an overlap becomes `InvalidStoredTaxTimeBandsException`, and invalid Tax Exemption content becomes `InvalidStoredTaxExemptionException` at this boundary. `CalculationExceptionHandler` maps calculation, deserialization, Bean Validation, unreadable-request, and unexpected exceptions to Problem Details. The Domain and Tax Rule areas do not depend on Spring Web.

The HTTP exception boundary logs each handled `4xx` response at `WARN` with a stable failure category, safe context, and no stack trace. It logs each handled `5xx` response once at `ERROR` with an internal stack trace. Invalid stored Charge Window or Daily Maximum content uses the `invalid-tax-rule-option` category. Invalid Tax Exemption content uses `invalid-tax-exemption`. An empty Tax Time Band collection or a non-exempt Passage with no matching Tax Time Band uses the `missing-tax-time-bands` category. An invalid stored City time zone uses `invalid-city-time-zone`. Overlapping Tax Time Bands use `overlapping-tax-time-bands`. This rule applies to the calculation API exception handler, not to unrelated framework or servlet responses.
