...
`CalculationServiceImpl` owns supported-year input failures. It also translates lower lookup and stored-content failures into calculation-owned exceptions and preserves their causes. Missing Tax Time Bands become `MissingStoredTaxTimeBandsException`, an invalid City time zone becomes `InvalidStoredCityTimeZoneException`, an overlap becomes `InvalidStoredTaxTimeBandsException`, and invalid Tax Exemption content becomes `InvalidStoredTaxExemptionException` at this boundary. `CalculationExceptionHandler` maps calculation, deserialization, Bean Validation, unreadable-request, and unexpected exceptions to Problem Details. The domain and Tax Rule areas do not depend on Spring Web.

The HTTP exception boundary logs each handled `4xx` response at `WARN` with a stable failure category, safe context, and no stack trace. It logs each handled `5xx` response once at `ERROR` with an internal stack trace. Invalid stored Charge Window or Daily Maximum content uses the `invalid-tax-rule-option` category. Invalid Tax Exemption content uses `invalid-tax-exemption`. Missing Tax Time Bands use `missing-tax-time-bands`. An invalid stored City time zone uses `invalid-city-time-zone`. Overlapping Tax Time Bands use `overlapping-tax-time-bands`. This rule applies to the calculation API exception handler, not to unrelated framework or servlet responses.
