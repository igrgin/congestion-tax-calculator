...
**Assumption used:** No. Each stored set of rules has an effective date and does not change after use. A rule change creates a new version.

**Result:** The application selects the latest version that started on or before the date being calculated. Historical versions remain available.
